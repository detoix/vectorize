...
- real-x: overall horizontal width
- real-y: overall vertical depth
The footprint is the axis-aligned bounding box of the union of EXTERIOR walls (thick/dark wall strokes).
Include attached volumes that are physically connected by exterior walls (e.g., garage). Exclude detached site elements (paths, landscaping, fences).
...
B) If units are not labeled, infer from building-scale plausibility and drafting conventions:
   - Convert each candidate assuming mm, cm, and m, and keep only interpretations where each axis is plausibly 3–60 meters for a residential footprint.
...
{
  "real-x": { "value": <number>, "unit": "<mm|cm|m>" },
  "real-y": { "value": <number>, "unit": "<mm|cm|m>" }
}

Notes:
- "value" must be numeric (float).
- "unit" must be exactly one of: "mm", "cm", "m".
